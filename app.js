...
(function initSplash() {
  ...
  const chars = '0123456789.,$+-/%#*=:;()&@ABCDEFGHIJKLMNOPQRSTUVWXYZ006699EE';
  const particles = [];
  let w, h;
...
const CURRENCIES = [
  ...
  { group: 'MIDDLE EAST', items: [
    ...
    { v: 'KWD', l: 'KWD — KUWAITI DINAR' },
    { v: 'BHD', l: 'BHD — BAHRAINI DINAR' },
    { v: 'OMR', l: 'OMR — OMANI RIAL' },
    { v: 'JOD', l: 'JOD — JORDANIAN DINAR' },
    { v: 'ILS', l: 'ILS — ISRAELI SHEKEL (₪)' },
    { v: 'IQD', l: 'IQD — IRAQI DINAR' },
  ...
  { group: 'AFRICA', items: [
    ...
    { v: 'ETB', l: 'ETB — ETHIOPIAN BIRR' },
    { v: 'MAD', l: 'MAD — MOROCCAN DIRHAM' },
    { v: 'TND', l: 'TND — TUNISIAN DINAR' },
    { v: 'XOF', l: 'XOF — WEST AFRICAN CFA FRANC' },
    { v: 'XAF', l: 'XAF — CENTRAL AFRICAN CFA FRANC' },
    { v: 'RWF', l: 'RWF — RWANDAN FRANC' },
    { v: 'MZN', l: 'MZN — MOZAMBICAN METICAL' }
  ]}
];
const BASE_EMOTIONS = [
...
const SEED_DATA = [
  ...
  { id:'s10', storeType:'grocery', total:52.83, items:12, region:'eu-west', currency:'EUR', tags:['stress','scarcity','grief'], note:'Prices up again. My mother used to cook this for less than half.', date:'2026-03-08' },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initSearchableSelect('currency-select', CURRENCIES, v => currencyValue = v);
  renderArchive();
  renderStats();
...
function handleSubmit() {
  ...
  const entry = { id:'u'+Date.now(), storeType:storeValue, total, items, region:regionValue, currency:currencyValue, tags:Array.from(selectedTags), note, date:new Date().toISOString().split('T')[0] };
  saveEntry(entry);
  if (pendingPhoto) saveGalleryPhoto({ id:'p'+Date.now(), image:pendingPhoto, date:entry.date, timestamp:new Date().toISOString(), region:regionValue });
...
function resetForm() {
  storeValue=''; regionValue=''; currencyValue='USD';
...
function renderArchive() {
  ...
  grid.innerHTML = f.map((e,i)=>`
    <div class="archive-entry" style="animation-delay:${i*.04}s">
      <div class="entry-header"><span class="entry-store">${fmtStore(e.storeType)}</span><span class="entry-amount">${fmtCurrency(e.currency)}${e.total.toFixed(2)}</span></div>
...
function renderStats() {
  ...
  const currencies=new Set(entries.map(e=>e.currency||'USD').filter(Boolean)).size;
  el.innerHTML = `<div class="stat"><span class="stat-num">${n}</span><span class="stat-label">RECEIPTS</span></div>
    <div class="stat"><span class="stat-num">${currencies}</span><span class="stat-label">CURRENCIES</span></div>
...
function fmtCurrency(c) {
  const symbols = { USD:'$', EUR:'€', GBP:'£', JPY:'¥', CNY:'¥', INR:'₹', KRW:'₩', THB:'฿', TRY:'₺', ILS:'₪', NGN:'₦', BRL:'R$' };
